...
DnTable <- UHS[, dsra::fitModel.Dn.TR(Sa = Sa, PGA = PGA, Tn = Tn, Mw = 6.5, xD = 1.3, kymin = 0.001, kymax = 0.55, n = 100), by = .(ID, Tn, TR, Vref, Vs30, p)] |> unique()


# Example #3. Build KmaxTable
devtools::load_all()
DnTRmodel <- DnTable[, .(ID, SM, Tn, TR, p, Ts, Vs30, Vref, a, b, e, PGA)] |> unique()
KhTRmodel <- DnTRmodel[, dsra::fitModel.Kmax.TR(a = a, b = b, e = e, pga = PGA, Ts = Ts, n = 100), by = .(ID, SM, Tn, TR, p, Vs30, Vref)]
KhTRmodel[, dsra::fitModel.Kmax.Ts(.SD, Tso = 0.6, Dao = 1, model = "rf"), by = .(TR, p, Vs30), .SDcols = colnames(KhTRmodel)]
KhTRmodel[, dsra::fitModel.Kmax.Ts(.SD, Tso = 0.6, Dao = 1, model = "nlm"), by = .(TR, p, Vs30), .SDcols = colnames(KhTRmodel)]
