# rm(list=ls())
OutputFolder <- file.path("~/Public/Database/gmdp/output/disagg/BC") |> normalizePath()
HazardFolder <- file.path("~/Public/Database/gmdp/hazard") |> normalizePath()
...
OutputID <- sample(PENDING,size=1)
for(OutputID in sample(PENDING)){

  PATH <- file.path(OutputFolder,OutputID) |> normalizePath()
  dir.exists(PATH)
  GMDP <- gmdp(path=PATH,ID=OutputID,TRo = c(100,200,475,500,1000,2000,2475,2500,5000,10000),Vs30_STEP=10)
...
#
OutputID <- "ARM2J1W5"
OutputFolder <- file.path("~/Public/Database/gmdp/output/user") |> normalizePath()
HazardFolder <- file.path("~/Public/Database/gmdp/hazard") |> normalizePath()
dir.exists(OutputFolder)
dir.exists(HazardFolder)
PATH <- file.path(OutputFolder,OutputID) |> normalizePath()
GMDP <- gmdp(path=PATH,ID=OutputID,TRo = c(100,200,475,500,1000,2000,2475,2500,5000,10000))
message(sprintf("> Saving openquake model %s...",OutputID))
FILE <- file.path(HazardFolder,paste0("PSHA.",OutputID,".Rds"))
saveRDS(GMDP,file = FILE)
