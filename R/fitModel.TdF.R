...
fitModel.TdF <- function(Hmax,lo=NULL,Bmax=NULL,Bo=NULL,GravelsFraction=NULL,SandsFraction=NULL,FinesFraction=NULL,level=0.95){
  ...
  GoF <- fitModel.GoF(Hso=Hmax,GravelsFraction=GravelsFraction,SandsFraction=SandsFraction,FinesFraction=FinesFraction,level=level)
  VSoF <- fitModel.VSoF(Hso=Hmax,GravelsFraction=GravelsFraction,SandsFraction=SandsFraction,FinesFraction=FinesFraction,level=level)
  moF <- fitModel.moF(Hso=Hmax,GravelsFraction=GravelsFraction,SandsFraction=SandsFraction,FinesFraction=FinesFraction,level=level)
  an <- fitModel.an(mo=moF[["mean"]],lo=lo,level=level)

  Ts <- (4*pi*Hmax/(an*(2 - moF[["mean"]])*VSoF[["mean"]]))
  return(list(Ts=round(Ts,2),Go=GoF,VSo=VSoF,mo=moF,an=an))
